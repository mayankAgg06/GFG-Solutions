...
class Solution {
  public:
    Node* parentAndTarget(Node* root,unordered_map<Node*,Node*>& parents, int target)
    {
        queue<Node*> q;
        q.push(root);
        Node* start=NULL;
    ...
    int minTime(Node* root, int target) 
    {
        ...
        Node* start = parentAndTarget(root,parents,target);
        ...
        while(!q.empty())
        {
            ...
            while(size--)
            {
                ...
                if(parents[front] and !visited[parents[front]])
                {
